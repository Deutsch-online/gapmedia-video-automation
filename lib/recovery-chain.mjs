...
// `git diff --staged --quiet` exits 1 when something is staged, 0 when the
// index matches HEAD. Used so an empty index ends this step deliberately
// instead of through a failed `git commit`.
function hasStagedChanges() {
  try {
    execFileSync("git", ["diff", "--staged", "--quiet"], { stdio: "ignore" });
    return false;
  } catch {
    return true;
  }
...
if (isMain) {
  ...
  // TRIGGER is JSON written by the Cloudflare scheduled worker
  // (worker/src/index.js) since dispatch moved there. The retired
  // plain-text ping line this step used to rewrite is gone from that file,
  // so the regex matched nothing and the rewrite produced a byte-identical
  // file. MARKER is only ever read here, never written, so `git add` staged
  // nothing at all and `git commit` exited non-zero with "nothing to
  // commit" — which took the whole failure() step down with it and lost the
  // hand-off this script exists to perform (run #109, 2026-09-19).
  //
  // The trigger was never this script's to touch in any case: the header
  // above records, measured, that a GITHUB_TOKEN push cannot start another
  // run, so rewriting it bought nothing even when the format still matched.
  // Stage the evidence alone, and stand down quietly when there is none.
  git("add", "-f", MARKER);
  if (!hasStagedChanges()) {
    console.log(`${MARKER} is already on main unchanged — the evidence the next attempt needs is committed already, so this run has nothing further to push.`);
    process.exit(0);
  }
