 ...
 */
/**
 * A compact, non-guessing description of a page an engine returned but this
 * module could not read. Run #275 (2026-09-14) is why: the ledger said
 * "duckduckgo: 202, 0 parsed — markup not recognised", and 202 is neither a
 * refusal (403/429, which cool the engine) nor a normal 200. Two very
 * different things produce it — an anti-bot challenge page served instead of
 * results, or real results whose markup changed under our parser — and they
 * need opposite fixes. Rather than guess which, this records what the page
 * actually says: its size and the first words of its visible text.
 *
 * @param {string} html
 * @returns {string}
 */
export function bodySignature(html) {
  ...
  const text = raw
    ...
    .replace(/\s+/g, " ")
    .trim();
  return `${raw.length}B, page says: ${JSON.stringify(text.slice(0, 120))}`;
}
...
export async function webSearch(query, { limit = 8, engines = ENGINES } = {}) {
  ...
  for (const engine of engines) {
    ...
    try { body = await res.text(); }
      ...
      tried[tried.length - 1] = `${engine.name}(${res.status}, body unreadable)`;
      record(engine.name, `${res.status}, body unreadable: ${String(e?.message || e).slice(0, 40)}`);
      continue;
    }
    let results = [];
    let parseError = "";
    try { results = engine.parse(body).slice(0, limit); }
    ...
    // The ledger gets the page itself, not a verdict about it: "markup not
    // recognised" was an assumption, and a challenge page reads identically.
    record(engine.name, parseError
      ? `${res.status}, parse failed: ${parseError} — ${bodySignature(body)}`
      : `${res.status}, 0 parsed — ${bodySignature(body)}`);
  }
