...
export function resetEngineCooldown() { refused.clear(); }
...
export async function webSearch(query, { limit = 8, engines = ENGINES } = {}) {
  ...
  for (const engine of engines) {
    if (refused.has(engine.name)) continue;
    ...
    let results = [];
    try { results = engine.parse(await res.text()).slice(0, limit); } catch { results = []; }
    if (results.length) return { results, engine: engine.name, tried };
  }
