...
export async function judgeRelevance(topic, points, candidate) {
  const prompt = relevancePrompt(topic, points, candidate);
  const why = [];
  if (GEMINI_KEY) {
    try {
      const verdict = parseVerdict(await askGemini(prompt));
      if (verdict) return { relevant: verdict.relevant, judged: true };
      why.push("gemini: no parsable verdict");
    } catch (e) { why.push(`gemini: ${String(e?.message || e).slice(0, 60)}`); }
  } else why.push("gemini: no key");
  if (GROQ_KEY) {
    try {
      const verdict = parseVerdict(await askGroq(prompt));
      if (verdict) return { relevant: verdict.relevant, judged: true };
      why.push("groq: no parsable verdict");
    } catch (e) { why.push(`groq: ${String(e?.message || e).slice(0, 60)}`); }
  } else why.push("groq: no key");
...
export async function isRelevant(topic, points, candidate) {
  return (await judgeRelevance(topic, points, candidate)).relevant;
}
...
export async function findRealImage(topic, points = [], usedImageUrls = new Set()) {
  ...
  for (const [strategyIndex, buildQuery] of REAL_IMAGE_SEARCH_STRATEGIES.entries()) {
    ...
    if (!hits.length) {
      try {
        hits = await ddgImageHits(buildQuery(topic, cue));
    ...
    const rejected = { used: 0, download: 0, type: 0, size: 0, irrelevant: 0, unjudged: 0, bestSmall: null };
    ...
    for (const hit of examined) {
      ...
      if (!verdict.relevant) { rejected[verdict.judged ? "irrelevant" : "unjudged"]++; continue; }
    ...
    console.error(`   ⚠ findRealImage(${topic}): strategy ${strategyIndex + 1}/${REAL_IMAGE_SEARCH_STRATEGIES.length} — ${examined.length} candidates examined${suffix}, none passed — used:${rejected.used} download:${rejected.download} badType:${rejected.type} tooSmall:${rejected.size} notRelevant:${rejected.irrelevant} judgeUnavailable:${rejected.unjudged}${nearMiss}${judgeWhy}`);
  }
