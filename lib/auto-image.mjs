...
export async function findRealImage(topic, points = [], usedImageUrls = new Set()) {
  ...
  for (const [strategyIndex, buildQuery] of REAL_IMAGE_SEARCH_STRATEGIES.entries()) {
    ...
    const rejected = { used: 0, download: 0, type: 0, size: 0, irrelevant: 0, unjudged: 0 };
    ...
    console.error(`   ⚠ findRealImage(${topic}): strategy ${strategyIndex + 1}/${REAL_IMAGE_SEARCH_STRATEGIES.length} — ${examined.length} candidates examined${suffix}, none passed — used:${rejected.used} download:${rejected.download} badType:${rejected.type} tooSmall:${rejected.size} notRelevant:${rejected.irrelevant} judgeUnavailable:${rejected.unjudged}`);
  }
