...
export async function findRealImage(topic, points = [], usedImageUrls = new Set()) {
  ...
  for (const [strategyIndex, buildQuery] of REAL_IMAGE_SEARCH_STRATEGIES.entries()) {
    ...
    console.error(`   ⚠ findRealImage(${topic}): strategy ${strategyIndex + 1}/${REAL_IMAGE_SEARCH_STRATEGIES.length} — ${examined.length} candidates examined${suffix}, none passed — used:${rejected.used} download:${rejected.download} badType:${rejected.type} tooSmall:${rejected.size} notRelevant:${rejected.irrelevant} judgeUnavailable:${rejected.unjudged}${nearMiss}${judgeWhy}${stopped}`);
  }
