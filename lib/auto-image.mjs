...
export async function judgeRelevance(topic, points, candidate) {
  ...
  return { relevant: false, judged: false };
}
...
export async function findRealImage(topic, points = [], usedImageUrls = new Set()) {
  ...
  for (const [strategyIndex, buildQuery] of REAL_IMAGE_SEARCH_STRATEGIES.entries()) {
    ...
    for (const hit of examined) {
      ...
      try { verdict = await judgeRelevance(topic, points, candidate); } catch { verdict = { relevant: false, judged: false }; }
    ...
    console.error(`   ⚠ findRealImage(${topic}): strategy ${strategyIndex + 1}/${REAL_IMAGE_SEARCH_STRATEGIES.length} — ${examined.length} candidates examined${suffix}, none passed — used:${rejected.used} download:${rejected.download} badType:${rejected.type} tooSmall:${rejected.size} notRelevant:${rejected.irrelevant} judgeUnavailable:${rejected.unjudged}${nearMiss}`);
  }
