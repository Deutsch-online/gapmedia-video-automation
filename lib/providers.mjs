...
export const PROVIDERS = Object.freeze({
  ...
  video: [
    // Google Flow is the creative web product.  The supported programmatic
    // route is Gemini API's Veo endpoint, intentionally off until the owner
    // enables it explicitly — a text/image video generation can be billable.
    { id: "google-veo", name: "Google Flow / Veo", type: "API / اختیاری", requires: ["GOOGLE_VEO_API_KEY"], enabledWhen: (env) => env.ENABLE_GOOGLE_VEO === "true", purpose: "کلیپ ویدیویی واقعی برای موشن" },
  ],
  voice: [
...
function isReady(provider, env = process.env) {
  ...
  if (typeof provider.enabledWhen === "function" && !provider.enabledWhen(env)) return false;
  return true;
}
...
export function formatProviderReportFa(env = process.env) {
  ...
  const labels = { search: "جست‌وجو", text: "متن و ترجمه", video: "ویدیوی هوش مصنوعی", voice: "صدا", delivery: "ارسال" };
