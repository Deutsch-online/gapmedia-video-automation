 ...
 */
export async function firstVerifiedCandidate(hits, { prefix, query, label, provider, quality = {} }) {
  if (!hits?.length) {
  ...
  // The consumer declares its visual brief BEFORE any provider is queried.
  // A vocabulary photograph is enlarged and cropped to a 9:16 lesson card,
  // so accepting the generic 1080px floor first and discovering softness in
  // the renderer is backwards.  The default keeps existing app/news callers
  // unchanged; the German lesson lane requests the stricter brief below.
  const requiredLongSide = Number(quality.minLongSide) || MIN_LONG_SIDE;
  const requiredArea = Number(quality.minArea) || MIN_AREA;
  const requiredShortSide = Number(quality.minShortSide) || 0;
  ...
  for (const hit of hits) {
    ...
    if (!size || Math.max(size.width, size.height) < requiredLongSide || size.width * size.height < requiredArea || Math.min(size.width, size.height) < requiredShortSide) {
      rejected.size++;
    ...
    try {
      // lane: "vocabulary" — every caller of this file (findPexelsImage,
      // findWikimediaImage) serves the German A1 word list, never the
      // app-feature pipeline, so the claim being judged is always "does this
      // picture show what this word means". Judging it with the feature
      // prompt is what rejected 8 of 8 real photographs per word on run #342.
  ...
  const spec = `spec:${requiredLongSide}px/${requiredArea}px${requiredShortSide ? `/${requiredShortSide}px-short` : ""}`;
  ...
  console.error(`   ⚠ ${provider}(«${query}»): ${examined} candidates examined, none passed — ${spec} · download:${rejected.download} badType:${rejected.type} tooSmall:${rejected.size} notRelevant:${rejected.irrelevant} judgeUnavailable:${rejected.unjudged}${nearMiss}${down}${stopped}`);
  return null;
}
