...
import { exaSearch, exaConfigured, isRelevant, generateAIImage, generateLocalFallbackImage, freeCatalogueImageHits } from "./auto-image.mjs";
...
import { firstVerifiedCandidate, withTimeout } from "./image-candidates.mjs";

// This is the visual-search contract for a lesson BEFORE the first request.
// A vertical 1080×1920 export zooms into each photo; 576×1024 social cards
// and small search thumbnails look soft even if they technically decode.
// The sources are free/licence-clean and are tried as an actual cascade, not
// a single-provider gamble: Pexels → Openverse/Wikimedia Commons → German
// Wikipedia → optional Exa → labelled AI fallback → our local graphic.
export const LESSON_IMAGE_CRITERIA = Object.freeze({
  minLongSide: 1440,
  minShortSide: 900,
  minArea: 1_400_000,
  target: "portrait 9:16 crop-safe real photograph",
});

function criteriaNote() {
  return `${LESSON_IMAGE_CRITERIA.target}; ≥${LESSON_IMAGE_CRITERIA.minLongSide}px long side; ≥${LESSON_IMAGE_CRITERIA.minShortSide}px short side; ≥${LESSON_IMAGE_CRITERIA.minArea.toLocaleString("en-US")} pixels`;
}

async function findCatalogueImage(query, label) {
  let hits = [];
  try {
    hits = await freeCatalogueImageHits(query);
  } catch (e) {
    console.error(`   ⚠ Openverse/Wikimedia Commons(«${query}»): ${e.message}`);
    return null;
  }
  return firstVerifiedCandidate(
    hits.map((hit) => ({ ...hit, credit: hit.provider || "Openverse" })),
    { prefix: "catalogue", query, label, provider: "Openverse/Wikimedia Commons", quality: LESSON_IMAGE_CRITERIA },
  );
}
...
async function attempt(searchQuery, relevanceTopic, label) {
  ...
  for (const hit of hits.slice(0, 10)) {
    ...
    if (!size || Math.max(size.width, size.height) < LESSON_IMAGE_CRITERIA.minLongSide || size.width * size.height < LESSON_IMAGE_CRITERIA.minArea || Math.min(size.width, size.height) < LESSON_IMAGE_CRITERIA.minShortSide) continue;
...
export async function findLessonImage(query, label, ownAssetKey = "") {
  ...
  console.error(`   ℹ [lesson-image] criteria locked before search: ${criteriaNote()}`);
  ...
  for (const phrase of queryTiers) {
    const fromPexels = await findPexelsImage(phrase, label, LESSON_IMAGE_CRITERIA);
    if (fromPexels) return fromPexels;
  }

  // LAYER 2: a second independent, free and licence-clean image catalogue.
  // This is deliberately before Wikipedia article lead photos: Openverse and
  // Commons hold original photographs, while an article lead can be a logo,
  // diagram or a too-small editorial crop.
  for (const phrase of queryTiers) {
    const fromCatalogue = await findCatalogueImage(phrase, label);
  ...
  for (const phrase of queryTiers) {
    const fromWikimedia = await findWikimediaImage(phrase, label, LESSON_IMAGE_CRITERIA);
