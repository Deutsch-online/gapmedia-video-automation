...
export function telegramConfig(env = loadEnv()) {
  ...
  // Owner directive, 2026-09-19, repeated: a finished VIDEO goes to the bot
  // chat, never to the channel. Reports and prompts may still go wherever
  // TELEGRAM_CHAT_ID points, so the video destination is its own setting
  // rather than a reinterpretation of that one. When it is unset the video
  // falls back to TELEGRAM_CHAT_ID, and verifyDelivery refuses that fallback
  // if it is not a private chat — so the rule holds either way.
  // The chat the bot has LEARNED wins over the configured one, because the
  // configured one is the channel and the owner asked for the bot instead
  // (2026-09-20). Resolved lazily to keep this module free of a cycle:
  // lib/bot-chat.mjs imports getUpdates from here.
  let reviewChatId = env.TELEGRAM_REVIEW_CHAT_ID;
  if (!reviewChatId) {
    try {
      const saved = JSON.parse(readFileSync(".telegram-bot-chat.json", "utf8"));
      if (saved?.id) reviewChatId = String(saved.id);
    } catch { /* not learned yet */ }
  }
  reviewChatId = reviewChatId || chatId;
  return { enabled: Boolean(token && chatId), token, chatId, reviewChatId };
}
...
export async function verifyDelivery({ token, chatId, requirePrivate = false }) {
  ...
  // Owner directive, 2026-09-19: a finished lesson goes to the BOT chat so a
  // person can watch it first. It never goes straight to a broadcast channel.
  // Publishing is a human decision, and a video that reaches a channel cannot
  // be un-published, so a channel destination is refused HERE — before the
  // build — rather than discovered once subscribers already have it.
  // Strictest form of the same directive: a lesson video must land in the
  // one-to-one chat with the bot. A group is not the channel, but it is not
  // the bot chat either, and the instruction named the bot.
  if (requirePrivate && chat.result?.type && chat.result.type !== "private") {
    const name = chat.result.title || target;
    return {
      ok: false,
      reason: `مقصد ویدیو «${name}» از نوع «${chat.result.type}» است، نه گفت‌وگوی خصوصی با ربات. طبق قانون پروژه، ویدیوی آماده فقط به گفت‌وگوی خودتان با ربات می‌رود تا اول ببینیدش. TELEGRAM_REVIEW_CHAT_ID را روی شناسهٔ عددی خودتان بگذارید؛ «node get-chat-id.mjs» آن را نشان می‌دهد.`,
      chat: chat.result,
    };
  }
  if (chat.result?.type === "channel") {
    ...
    return {
      ok: false,
      reason: `TELEGRAM_CHAT_ID به یک کانال اشاره می‌کند («${name}»). طبق قانون پروژه، ویدیوی آماده باید به گفت‌وگوی خصوصی با ربات برود تا اول بازبینی شود، نه مستقیم به کانال. Chat ID را روی گفت‌وگوی خودتان با ربات بگذارید؛ «node get-chat-id.mjs» آن را نشان می‌دهد. انتشار در کانال کار شماست، نه کار خودکار.`,
      chat: chat.result,
    };
  }
...
export async function getUpdates({ token, offset, timeout = 50 }) {
  // `offset` is optional: a caller that only wants to LOOK at the queue omits
  // it, so nothing is confirmed and the real poller keeps its place. It used to
  // be interpolated unconditionally, so omitting it sent «offset=undefined»,
  // which Telegram rejects as a bad integer — the call came back ok:false and
  // this returned [], indistinguishable from an empty queue. That is why the
  // bot-chat discovery reported "no private chat has written to the bot yet"
  // for a chat that had in fact written (run #137, 2026-09-20).
  const params = new URLSearchParams({ timeout: String(timeout) });
  if (Number.isFinite(Number(offset)) && offset !== null && offset !== undefined && offset !== "") {
    params.set("offset", String(Number(offset)));
  }
  const res = await fetch(`https://api.telegram.org/bot${token}/getUpdates?${params}`);
  const json = await res.json().catch(() => ({ ok: false }));
  // A rejected call is not an empty queue, and a caller deciding whether a chat
  // exists must be able to tell them apart.
  if (!json.ok) throw new Error(`Telegram getUpdates failed: ${json.description || "unknown error"}`);
  return json.result || [];
}
